    ...
    companion object {
        ...
        init {
            ...
            if (Build.VERSION.SDK_INT < Build.VERSION_CODES.O) {
                ...
                try {
                    com.ando.toolkit.fileInputStream =
                        FileInputStream(File(Environment.getRootDirectory(), "build.prop"))
                    com.ando.toolkit.properties.load(com.ando.toolkit.fileInputStream)
                } catch (e: Exception) {
                    e.printStackTrace()
                } finally {
                    closeIOQuietly(com.ando.toolkit.fileInputStream)
                }
            }
            val clzSystemProperties: Class<*>? = null
            try {
                com.ando.toolkit.clzSystemProperties = Class.forName("android.os.SystemProperties")
                val getMethod: Method = com.ando.toolkit.clzSystemProperties.getDeclaredMethod(
                    "get",
                    String::class.java
                )
                // miui
                sMiuiVersionName = getLowerCaseName(
                    com.ando.toolkit.properties,
                    com.ando.toolkit.getMethod,
                    KEY_VERSION_MIUI
                )
                //flyme
